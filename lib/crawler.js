...
//new
Crawler.prototype.new = function (url) {
	var that = this;
	that.log('抓取文本中...');
			...
			pool.getConnection(function(err, connection) {
				...
				newList.map(function(el){
					var _Promise = new Promise(function(resolve,reject){
						connection.query(`SELECT * from t_new_list where content='${el.content}'`,function(err,rows){
							if(err) return that.log(`查询${el.content}是否存在报错+${err}`,'red'); reject(err)
							if(rows instanceof Array && rows.length === 0){
								connection.query(`INSERT INTO t_new_list SET ?`,el,function(err,rows){
									if(err) return that.log(`插入${el.content}时报错,${err}`,'red'); reject(err)
